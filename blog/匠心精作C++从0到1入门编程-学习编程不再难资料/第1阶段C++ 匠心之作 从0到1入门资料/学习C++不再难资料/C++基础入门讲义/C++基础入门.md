...

{
	...
	cout << "请输入小猪3的体重" << endl;
	cin >> pig3;
	cout << "请输入小猪1的体重是：" << pig1 << endl;
	cout << "请输入小猪2的体重是：" << pig2 << endl;
	cout << "请输入小猪3的体重是：" << pig3 << endl;
	if (pig1 > pig2)
	{
		if (pig1 > pig3)
		{
...
> 总结1：数组名的命名规范与变量名命名规范一致，不要和变量重名  定义数组的时候 必须有初始长度！
...
**函数定义**就像后厨做菜的全过程（洗菜、炒菜、装盘）
**函数声明**就像菜单上的菜品介绍（菜名、价格、辣度提示）
...
1. **顾客不用冲进后厨**：
   你不需要知道宫保鸡丁具体怎么做（定义），只要看菜单（声明）就知道有没有这道菜、多少钱、辣不辣。
2. **避免点错菜**：
   如果菜单写「宫保鸡丁-微辣」，结果后厨做成变态辣——这就是声明和定义不一致，编译器会报错提醒你。
...
int main() {
	...
	//指针变量赋值
	p = &a; //让 指针指向变量a的地址 获取变量的地址
	cout << &a << endl; //打印数据a的地址
	cout << p << endl;  //打印指针变量p 一样的地址和&a

	//2、指针的使用
	//通过*操作指针变量指向的内存 代表找到了p指向的内存的地址
	cout << "*p = " << *p << endl;
...
2. const修饰常量   --- 指针常量 指针指向不可改 指向的值可改
3. const即修饰指针，又修饰常量  指向和值都不可改
...
**结构体就是个「自定义数据收纳包」**，用大白话解释就是：

**1️⃣ 现实类比**：
想象你要寄快递：

- **普通变量** → 单独寄送物品（比如只寄一本书）
- **结构体** → 打包一个快递箱（里面可以同时放书、衣服、零食）
...
//const使用场景
void printStudent(const student *stu) //加const防止函数体中的误操作  使用指针地址传递节省空间
{
